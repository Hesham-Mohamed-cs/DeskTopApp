    ...
    public partial class CasherForm : Form
    {
        ...
        private void Btn_Rmove_Click(object sender, EventArgs e)
        {
            ...
            if (count == 1)
            {
                ...
                catch
                {
                    ...
                    MessageBox.Show("Please add one item from bill", "error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
        ...
        private void Btn_Save_Click(object sender, EventArgs e)
        {
            ...
            for (int r = 0; r < BillGV.RowCount - 1; r++)
            {
                ...
                if (BillGV.Rows[r].Cells[0].Value == null)
                {
                    MessageBox.Show("Please inter item in  mune bill", "error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                var name = BillGV.Rows[r].Cells[0].Value.ToString();
                if (BillGV.Rows[r].Cells[2].Value == null)
                {
                    MessageBox.Show("Please inter item in  mune bill", "error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                var amount = Convert.ToInt32(BillGV.Rows[r].Cells[2].Value.ToString());
                if (BillGV.Rows[r].Cells[3].Value == null)
                {
                    MessageBox.Show("Please inter item in  mune bill", "error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                var price = Convert.ToDecimal(BillGV.Rows[r].Cells[3].Value);
                if (BillGV.Rows[r].Cells[1].Value == null)
                {
                    MessageBox.Show("Please inter item in  mune bill", "error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                var unitprice = Convert.ToDecimal(BillGV.Rows[r].Cells[1].Value);

                var food = con.Foods.Where(f => f.Name == name && f.Price == unitprice && f.UniteOnStock >= amount).FirstOrDefault();
                if (food == null)
                {
                    var Drink = con.Drinks.Where(f => f.Name == name && f.Price == unitprice && f.UniteOnStock >= amount).FirstOrDefault();
                    if (Drink == null)
                    {
                        MessageBox.Show($" {name} item is not found mune bill", "error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
                }
                if (unitprice * amount != price)
                {
                    MessageBox.Show("Please inter valid in  mune bill", "error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
        ...
        private void FoodCategory_Paint(object sender, PaintEventArgs e)
        {
